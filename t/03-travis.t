...
use Test::Sbotools qw/ sbopconfig sbopsnap sbopfind sbopinstall sbopremove sbopcheck sbopupgrade /;
...
sbopconfig qw/ -V 14.1 /, { expected => "Setting SLACKWARE_VERSION to 14.1...\n" };
SKIP: {
	skip 'Not doing online tests without TEST_ONLINE=1', 2 if $ENV{TEST_ONLINE} ne '1';

	sbopsnap 'fetch', { expected => qr/\APulling SlackBuilds tree\.\.\.\n/ };
	sbopfind 'sbotools', { expected => qr"SBo:    sbotools \d[.]\d\nPath:   /usr/sbo/repo/system/sbotools\n\n" };
}
...
sbopconfig qw! -r https://github.com/Ponce/slackbuilds.git !, { expected => "Setting REPO to https://github.com/Ponce/slackbuilds.git...\n", name => 'Alternative REPO' };
SKIP: {
	skip 'Not doing online tests without TEST_ONLINE=1', 4 if $ENV{TEST_ONLINE} ne '1';

	sbopsnap 'fetch', { expected => qr!Pulling SlackBuilds tree.*Cloning into '/usr/sbo/repo'!s };
	...
	sbopfind 'sbotools', { expected => qr"SBo:    sbotools .*\nPath:   /usr/sbo/repo/system/sbotools\n\n" };
}

# 11-17: Test local overrides
sbopconfig '-o', "$RealBin/LO", { expected => "Setting LOCAL_OVERRIDES to $RealBin/LO...\n", name => 'LOCAL_OVERRIDES' };
my $skip = 0;
SKIP: {
	...
	sbopfind 'nonexistentslackbuild', { expected => sub {
...
Path:   $RealBin/LO/nonexistentslackbuild8!
	...
	sbopinstall qw/ -r nonexistentslackbuild /,
		{ expected => qr/nonexistentslackbuild added to install queue[.].*perf[.]dummy.* saved.*Cleaning for nonexistentslackbuild-1[.]0/s };
	sbopremove qw/ --nointeractive nonexistentslackbuild /, { expected => qr/Removing 1 package\(s\).*nonexistentslackbuild.*All operations have completed/s };
	is (system(qw!touch /var/log/packages/nonexistentslackbuild-0.9-noarch-1_SBo!), 0, 'Old version fake installed');
	sbopcheck { expected => qr/Updating SlackBuilds tree.*Checking for updated SlackBuilds.*nonexistentslackbuild 0[.]9.*needs updating/s };
	sbopupgrade qw/ -r nonexistentslackbuild /, { expected => qr/nonexistentslackbuild added to upgrade queue.*Upgrade queue: nonexistentslackbuild/s };

# 18: Test missing dep
	sbopinstall 'nonexistentslackbuild2', { input => 'n', exit => 0, expected => "Unable to locate nonexistentslackbuild3 in the SlackBuilds.org tree.\nDo you want to ignore it and continue? [n] " };
}

# 19-23: Test sbopupgrade --all
SKIP: {
...
SKIP: {
	...
	sbopupgrade qw/ -r --all /, { expected => qr/Checking for updated SlackBuilds.*nonexistentslackbuild added to upgrade queue.*Cleaning for nonexistentslackbuild/s };
...
# 26: Test sbopupgrade -f -z
sbopupgrade qw/ -f -z nonexistentslackbuild4 /, {
	input => "y\ny\ny",
