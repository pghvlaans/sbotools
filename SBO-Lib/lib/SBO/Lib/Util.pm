...
C<sbotools> color display can be disabled by setting C<NOCOLOR> to C<FALSE>. C<$color_default>
...
C<NOCOLOR> setting is not C<TRUE>. See the C<Function Interface> section in
...
Additionally, C<BUILD_IGNORE>, C<RSYNC_DEFAULT> and C<NOCOLOR> are turned on
if C<CLASSIC> is C<TRUE>.
...
afterwards. No colors are used unless C<NOCOLOR> is C<FALSE>. A newline is added
