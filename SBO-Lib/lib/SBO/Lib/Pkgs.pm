...
use SBO::Lib::Util qw/ :const :config build_cmp in is_elf parse_readelf script_error error_code open_read version_cmp /;
...
It is generated by C<readelf_check()> via C<update_known_solibs()> if it is empty at the
time of calling.

=head2 %old_libs

A hash with a per-package list of apparently missing first-order shared object dependencies.
It is generated when running C<readelf_check()>.
...
=head2 readelf_check
  ...
  my $readelf_good = readelf_check($pkg);

C<readelf_check()> takes the name of a package file and checks for the presence
of any required first-order shard object dependencies as based on the output
of C<readelf(1)> and the C<@native_libs> shared object array. It returns 1 if all
required shared objects appear to be present and 0 otherwise.

Because C<is_elf()> is called (see C<SBO::Lib::Util(3)>), performance is cache-dependent.
For this reason, it is best to call C<readelf_check()> for C<SBO> packages only, and not
every package installed to the system.

=cut

sub readelf_check {
  script_error("readelf_check requires an argument.") unless @_ == 1;
  my $pkg = shift;
  ...
  for my $line (<$fh>) {
    ...
    chomp($line);
    next unless -f -x "/$line" or $line =~ m/\.so(|\.d+(|\.d+))$/;
    push @file_list, "/$line";
  }
  close $fh;
  undef $fh;
  my (@nonexistent, @candidate_files, @x86_candidates);
  for my $file (@file_list) {
    next unless my $is_elf = is_elf($file);
    if ($is_x86_64) {
      push @candidate_files, $file if $is_elf gt 0;
      push @x86_candidates, $file if $is_elf lt 0;
    } else {
      push @candidate_files, $file;
    }
  }
  undef @file_list;
  return 1 unless @candidate_files or @x86_candidates;
  my ($check_string, $x86_string, @shared, @x86_shared);
  if (@candidate_files) {
    $check_string = '"'. join("\" \"", @candidate_files). '"';
    @shared = split "\n", `/usr/bin/readelf -d $check_string 2> /dev/null | sort -u`;
  }
  if (@x86_candidates) {
    $x86_string = '"'. join("\" \"", @x86_candidates). '"';
    @x86_shared = split "\n", `/usr/bin/readelf -d $x86_string 2> /dev/null | sort -u`;
  }
  for my $cand (@shared) {
    next unless my $cand_lib = parse_readelf($cand);
    next if in $cand_lib, @native_libs;
    push @nonexistent, "\t$cand_lib" unless solib_present($cand_lib, $pkg, @candidate_files);
  }
  for my $cand (@x86_shared) {
    next unless my $cand_lib = parse_readelf($cand);
    next if in $cand_lib, @x86_libs;
    push @nonexistent, "\t$cand_lib (x86)" unless solib_present($cand_lib, $pkg, @x86_candidates);
  }
  undef @candidate_files;
  undef @x86_candidates;
  undef @shared;
  undef @x86_shared;
  if (@nonexistent) {
    $old_libs{$pkg} = join("\n", @nonexistent);
...
a package file and an array with known ELF files shipped by that package. It returns 1 if
...
sub solib_present {
  ...
  my ($cand_lib, $pkg, @candidate_files) = @_;
  return 1 if $cand_lib =~ m/^\// and -f $cand_lib;
  my $grep_lib = $cand_lib;
  $grep_lib =~ s/\+/\\+/;
  return 1 if grep { /\/$grep_lib$/ } @candidate_files;
  my $found_in_script;
  if (-f "$script_db/$pkg") {
...
SBO::Lib(3), SBO::Lib::Build(3), SBO::Lib::Download(3), SBO::Lib::Info(3), SBO::Lib::Readme(3), SBO::Lib::Repo(3), SBO::Lib::Tree(3), SBO::Lib::Util(3), ldconfig(1), readelf(1)
